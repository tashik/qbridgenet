...
public class Trade
{
    ...
    public string order_num {get; set;} = string.Empty; // Номер заявки в торговой системе
    public string brokerref {get; set;} = string.Empty; // Комментарий, обычно: <код клиента>/<номер поручения>
    public string userid {get; set;} = string.Empty; // Идентификатор трейдера
    public string firmid {get; set;} = string.Empty; // Идентификатор дилера
    public int canceled_uid {get; set;} // Идентификатор пользователя, отказавшегося от сделки
    public string account {get; set;} = string.Empty; // Торговый счет
    ...
    public string cpfirmid {get; set;} = string.Empty; // Код фирмы партнера
    ...
    public string client_code {get; set;} = string.Empty; // Код клиента
    ...
    public string settle_date {get; set;} = string.Empty; // Дата расчетов
    public string settle_currency {get; set;} = string.Empty; // Валюта расчетов
    public string trade_currency {get; set;} = string.Empty; // Валюта
    public string exchange_code {get; set;} = string.Empty; // Код биржи в торговой системе
    public string station_id {get; set;} = string.Empty; // Идентификатор рабочей станции
    ...
    public string bank_acc_id {get; set;} = string.Empty; // Идентификатор расчетного счета/кода в клиринговой организации
    ...
    public string linked_trade {get; set;} = string.Empty; // Номер витринной сделки в Торговой Системе для сделок РЕПО с ЦК и SWAP
    public int period {get; set;} // Период торговой сессии. Возможные значения: «0» – Открытие; «1» – Нормальный; «2» – Закрытие
    public string trans_id {get; set;} = string.Empty; // Идентификатор транзакции
    ...
    public string clearing_bank_accid {get; set;} = string.Empty; // Идентификатор счета в НКЦ (расчетный код)
    public QuikDateTime? canceled_datetime {get; set;}	// Дата и время снятия сделки
    public string clearing_firmid {get; set;} = string.Empty; // Идентификатор фирмы - участника клиринга
    public string system_ref {get; set;} = string.Empty; // Дополнительная информация по сделке, передаваемая торговой системой
